...
def main():     
        ...
        for ds in datasets:
            ...
            for i in functions:
                    ...
                    ##########################
                    ## If there is an error with an roi or function, take a note and continue
                    
                    except Exception as e:
                        error_ds.append(str(ds.sample))
                        error_func.append(str(i))
                        error_e.append(e)
                        
                        current_datetime = datetime.datetime.now()
                        current_datetime = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
                        error_time.append(str(current_datetime))
        ...
        if error_ds != []:
            error_df = pd.DataFrame(zip(error_ds, error_func, error_e, error_time), columns=['ROI','Func','Error', 'Time'])
...
def moruetaHolmeAssociationMatrix(ds, df_annotations, colors, clusterNames, counts):
        ...
        # MIKE:
        # I've altered this code so that it now exludes any populations which have less than 4 cells.

        minimum_cells = 4
        
        if len(np.where(np.sum(counts,axis=0) < minimum_cells)) > 0:
            toExclude = list(np.where(np.sum(counts,axis=0) < minimum_cells)[0])
            O = np.delete(O,toExclude,0)
            clusterNames_MH = np.delete(clusterNames_MH,toExclude,0)
