...
| `bint` | `🟢` | `Core` | Runs `SpatialBiologyToolkit.scripts.basic_process_batch_integration`: direct Harmony (`harmonypy`) and/or BBKNN batch correction, neighbors/UMAP/Leiden, and QC UMAP export. | `${IMC_ENV_SEGMENTATION:-imc_segmentation}` | AnnData from `batch_integration.input_adata_path` or fallback (`general.anndata_path`); valid `batch_integration.batch_correction_obs` in `adata.obs` for Harmony/BBKNN modes. | Updated AnnData at `batch_integration.output_adata_path` (default `general.anndata_path`) plus `QC/BatchIntegration/` UMAPs by default. | `general`, `batch_integration`, `logging` | After `nimbus`; alternative to `rapids`/`bbn` before `subcl`/`cchar`/`pairsp`/`nxsp`/`aiinter`/`vis`. |
| `rapids` | `🟢` | `Core` | Runs `SpatialBiologyToolkit.scripts.basic_process_rapids`: optional obs-based cell filtering, GPU PCA, optional RAPIDS Harmony integration, RAPIDS neighbors/UMAP/Leiden, QC UMAP + Leiden MatrixPlot export, and optional parameter scans. | `${IMC_ENV_RAPIDS_SINGLECELL:-rapids_singlecell}` | AnnData from `rapids.input_adata_path` or fallback (`general.anndata_path`); valid `rapids.batch_correction_obs` in `adata.obs` when `rapids.run_harmony=true`; optional `rapids.filter_obs_key` bounds. | Updated AnnData at `rapids.output_adata_path` (default `general.anndata_path`) for normal runs; parameter scans write QC only by default under `QC/RapidsProcess/ParameterScan/`, plus `QC/RapidsProcess/` UMAPs and `QC/RapidsProcess/Matrixplots/` for normal runs. | `general`, `rapids`, `visualization`, `logging` | After `nimbus`; alternative to `bint`/`bbn` before `subcl`/`cchar`/`pairsp`/`nxsp`/`aiinter`/`vis`. |
| `bbn` | `🟢` | `Core` | Runs `SpatialBiologyToolkit.scripts.basic_process_biobatchnet`: BioBatchNet batch correction, neighbors/UMAP/Leiden, saves processed AnnData and QC UMAPs. | `${IMC_ENV_BIOBATCHNET:-imc_biobatchnet}` | AnnData from `biobatchnet.input_adata_path` or fallback (`general.anndata_path`); valid `biobatchnet.batch_correction_obs` in `adata.obs`. | Updated AnnData at `biobatchnet.output_adata_path` (default `general.anndata_path`, or scan-suffixed variants), `QC/BioBatchNet/` UMAPs + scan summary CSV. | `general`, `biobatchnet`, `logging` | After `nimbus`; alternative to `bint`/`rapids` before `subcl`/`cchar`/`pairsp`/`nxsp`/`aiinter`/`vis`. |
...
RAPIDS MatrixPlots use `visualization.matrixplot_vmax`, `visualization.figure_format`, and `visualization.save_high_res`.

RAPIDS cell filter and parameter scan example:
...
rapids:
  ...
  parameter_scan_dict:
    ...
    harmony_flavor: [harmony1, harmony2]
...
- The `rapids` stage expects an existing RAPIDS-compatible conda environment named `rapids_singlecell` unless `IMC_ENV_RAPIDS_SINGLECELL` is set.
