...
| `rapids` | `🟢` | `Core` | Runs `SpatialBiologyToolkit.scripts.basic_process_rapids`: GPU PCA, optional RAPIDS Harmony integration, RAPIDS neighbors/UMAP/Leiden, and QC UMAP + Leiden MatrixPlot export. | `${IMC_ENV_RAPIDS_SINGLECELL:-rapids_singlecell}` | AnnData from `rapids.input_adata_path` or fallback (`general.anndata_path`); valid `rapids.batch_correction_obs` in `adata.obs` when `rapids.run_harmony=true`. | Updated AnnData at `rapids.output_adata_path` (default `general.anndata_path`) plus `QC/RapidsProcess/` UMAPs and `QC/RapidsProcess/Matrixplots/` by default. | `general`, `rapids`, `visualization`, `logging` | After `nimbus`; alternative to `bint`/`bbn` before `subcl`/`cchar`/`pairsp`/`nxsp`/`aiinter`/`vis`. |
