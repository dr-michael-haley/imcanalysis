...
9. `pairsp` (optional pairwise spatial statistics stage)
10. `aiinter` (optional; requires `OPENAI_API_KEY`)
11. `vis`
12. `reint` (optional; only if removed-marker AnnData exists)
13. `zipqc` (optional packaging)

Example:

```bash
pl config prep denoise dnqc cellpose nimbus bbn cchar pairsp aiinter vis reint zipqc
```
...
- `pairsp` can also be run as a side analysis after `bbn` (and optionally after `cchar`/`aiinter`, depending on which population labels you want to analyze).
