...
9. `cchar` (optional spatial neighborhood stage; independent of `pairsp`)
10. `pairsp` (optional pairwise spatial statistics stage; independent of `cchar`)
11. `aiinter` (optional; requires `OPENAI_API_KEY`)
12. `vis`
13. `reint` (optional; only if removed-marker AnnData exists)
14. `zipqc` (optional packaging)

Example:

```bash
pl config prep denoise dnqc cellpose nimbus bbn subcl cchar pairsp aiinter vis reint zipqc
```
...
- `cchar` and `pairsp` are independent `Analysis` stages that both branch from `bbn` (no dependency between them).
