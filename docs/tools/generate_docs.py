...
def generate_api_reference(repo_root: Path, source_dir: Path) -> int:
    ...
    for filename, module_name in modules:
        ...
        # Git treats a line containing exactly seven equals signs as an
        # unresolved-conflict marker. Use an equally valid RST underline for
        # seven-character module names so generated pages pass diff checks.
        title_character = "-" if len(display_name) == 7 else "="
        title_bar = title_character * len(display_name)
        content = "\n".join(
            [
                ".. Generated by docs/tools/generate_docs.py; do not edit directly.",
                "",
