...
| `normalization_lower_threshold` | `float` | `0.0` | `advanced` | Default absolute lower threshold assigned to newly computed channel normalization rows. Marker-specific values loaded from a normalization CSV or legacy JSON take precedence. | - |
...
| `normalization_dict_path` | `Optional[str]` | `null` | `advanced` | Optional path to a preferred Nimbus CSV with marker, vmax, and lower_threshold columns. Relative paths resolve from the project root, this path takes precedence over reuse_saved_normalization, and an external source is not overwritten. | - |
