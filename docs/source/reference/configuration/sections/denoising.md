...
| `loss_function` | `Literal['I_divergence', 'mse', 'mse_relu']` | `I_divergence` | `advanced` | Masked-pixel data-fidelity loss: 'I_divergence' gives the Poisson-aware DeepSNiF objective; 'mse' and 'mse_relu' select the library's Noise2Void-style variants. | - |
...
| `weights_save_directory` | `Optional[str]` | `null` | `advanced` | Directory for per-channel .weights.h5 weights, normalization-range files, and optional loss histories; when unset, use trained_weights under the working directory. | - |
| `weights_name_template` | `str` | `weights_{channel}.weights.h5` | `advanced` | Per-channel DeepSNiF checkpoint filename template. It must contain '{channel}'; new training requires the .weights.h5 suffix, while is_load_weights also accepts legacy .hdf5 or .h5 weight-only files. | - |
| `is_load_weights` | `bool` | `False` | `advanced` | Load weights_<channel>.weights.h5 and its matching normalization-range file from the weights directory instead of generating patches and training a new channel model. | - |
...
| `network_size` | `Literal['small', 'normal']` | `small` | `advanced` | DeepSNiF U-Net capacity: 'small' uses the compact, faster network, while 'normal' uses the original larger residual U-Net. | - |
