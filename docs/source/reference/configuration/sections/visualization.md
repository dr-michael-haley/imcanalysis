...
| `backgating_population_overlay_comparison_images` | `List[Union[str, SpatialBiologyToolkit.config.models.BackgatingComparisonImageConfig]]` | `[]` | `advanced` | Side-by-side image folders or panel settings (folder, title, legend, interpolation, show_cell_outlines). Match ROI filenames, resize to the IMC grid, then share its crop. Images must already cover the same tissue extent and orientation. Missing/ambiguous matches are labelled; no extra scale bars are added. | - |
| `backgating_population_overlay_primary_title` | `Optional[str]` | `IMC` | `advanced` | Primary panel title when comparison images are enabled; null or an empty string hides it. | - |
| `backgating_population_overlay_title_fontsize` | `Optional[float]` | `null` | `advanced` | Shared IMC and comparison panel title size in points, independent of legend text. Null uses backgating_population_overlay_legend_fontsize. | - |
