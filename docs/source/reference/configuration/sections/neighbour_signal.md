...
| `population_obs` | `Union[str, List[str], NoneType]` | `null` | `basic` | Population observation name or list of names for independent homotypic/heterotypic NAF and population QC. | Null falls back to general.population_obs_primary. A list creates separate component layers and QC for each annotation, without recalculating total NAF. Missing annotations are reported as unknown. |
