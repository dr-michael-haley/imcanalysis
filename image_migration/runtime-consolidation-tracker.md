...
| Define the joint `sbt-tensorflow` candidate | waiting for user | Added an inactive repository-managed Python 3.10/TensorFlow 2.15.1 candidate based on the HyPERSTAC compatibility spine, pinned IMC-Denoise to immutable commit `0a1c936`, and registered version, import, stage-module, and CPU TensorFlow smoke tests. | Review the candidate, then generate its Linux lock and perform the clean install on CSF3 in the next approved phase. | All 37 environment-management tests passed; the repository validator passed with zero warnings; the smoke script compiled; `git diff --check` passed. `sbt env validate-spec tensorflow` reports only the intentionally missing Linux lock plus the expected pinned-VCS review warning. No lock was generated and no stage mapping was changed. | Current imcanalysis working tree | The next phase must establish a clean joint install before GPU or scientific parity testing. Existing `sbt-denoise` and `sbt-hyperstac` remain untouched rollback routes. |
...
- Run Cellpose-SAM model loading and representative small-image inference on a
  ...
  provenance.
- Generate the `sbt-tensorflow` Linux lock on CSF3, perform a clean joint
  installation, and run its registered CPU/import checks before changing any
  denoise or HyPERSTAC stage mapping.
