...
| Correct the RAPIDS/SpatialData Dask-generation conflict | complete | Aligned the candidate to RAPIDS 24.12, RAPIDS-singlecell 0.12.0 and Dask/Distributed 2024.11.2; added cuGraph/dask-cuDF checks, a full GPU clustering smoke artifact, and version-aware Harmony handling that never substitutes Harmony2. Subsequent solver and smoke evidence aligned the candidate to Python 3.11, scikit-image 0.24.0 and Setuptools below 81, and corrected stage smoke tests to use the same process isolation as SLURM. | Generated the corrected lock, performed the clean installation, approved a controlled three-package repair, and ran the strengthened registered checks on CSF3. Michael elected to treat RAPIDS consolidation as successful while real-workflow testing continues. | Final lock SHA-256 `9dd5d793b972dce0b69961a466b146ae1fc75ea865954475d6fd0b28305fbec6`. In-place repair installed Panel 1.9.4, Panel Core 1.9.4 and Setuptools 80.10.2. On 2026-08-18 all 12 registered smoke tests passed, including SpatialData, CellCharter, BioBatchNet, Nimbus, RAPIDS/cuGraph/dask-cuDF, exact version assertions, and five isolated SBT stage imports. | `0e0588f`; CSF3 logs `sbt-analysis-three-package-repair-20260818-114456.log` and `sbt-analysis-post-repair-tests-20260818-114456.log` | `sbt env compare` still reports a mixture of comparison false positives and genuine package-layer drift caused by the current Conda-lock-then-pip installation model. This is an environment-management/reproducibility issue, not a failed smoke test, and must be corrected separately without perturbing the working runtime. RAPIDS-singlecell 0.12 supports original Harmony, not Harmony2. |
| Correct RAPIDS-singlecell 0.12 Numba compatibility | waiting for user | Pinned Numba 0.60.0 in both dependency layers, strengthened registered and GPU validation, and recorded the production failure without changing the RAPIDS algorithm or CUDA generation. | Publish and pull the specification; archive and regenerate the Linux lock on CSF3; return the lock result for review before repairing the environment. | Production job `18823078` completed neighbours and UMAP for 1,279,364 cells but segfaulted when cuGraph Leiden initialized. The installed Numba 0.63.1 violated RAPIDS-singlecell 0.12.0's declared `numba<0.61.0` constraint. All 36 environment-management tests and the repository validator pass locally. | Pending Linux lock | Do not rerun the production RAPIDS stage until the corrected lock, environment repair, and GPU Leiden smoke test pass. The mixed CUDA 12.x package versions remain a secondary diagnostic only if the supported Numba set does not resolve the crash. |
| Run targeted end-to-end acceptance | in progress | Keep the managed runtime and stage records suitable for representative real workflows. | Resume the RAPIDS workflow after the Numba repair passes its focused GPU Leiden smoke test. | Job `18823078` established that neighbours and UMAP run at production scale, but exposed an unsupported Numba dependency at the Leiden boundary. | Pending corrected CSF3 run evidence | A failure after the documented Numba repair should be handled as a focused CUDA/cuGraph or scale investigation, not by silently changing algorithms. |
| Standardise environment names and archive superseded specifications | waiting for user | Renamed all active physical environments to the `sbt-*` convention; removed retired registry keys; remapped their stages to `analysis`; updated wrappers, diagnostics, tests and docs; archived the three repository-managed retired specifications and a pre-change registry snapshot under `image_migration/archive/retired_hpc_environments/`. | Transfer the exact final `sbt-analysis` lock from CSF3 into the repository, publish/pull this checkpoint, and recreate external environments under their registered `sbt-*` names when needed. Regenerate the denoising lock in a separate reviewed lock-maintenance step before relying on `sbt env sync denoise`. | 66 environment/run-control tests and 8 affected stage integration tests passed; all active shell wrappers pass `bash -n`; generated docs are current; repository validator passes; `sbt env list` exposes exactly eight standardized names; no active orchestration reference uses a retired physical Conda name or `IMC_ENV_*` runtime override. | Local working tree, not committed | `HPC_env_files/sbt-analysis/conda-linux-64.lock` is missing from this checkout. Its required SHA-256 is `9dd5d793b972dce0b69961a466b146ae1fc75ea865954475d6fd0b28305fbec6`. The retained denoising lock contains legacy pip records and fails the current specification validator. Existing HPC environments are not removed automatically. |
| Repair `sbt-cellpose-sam` reproducibility | waiting for user | Restored the exact exported Torch 2.9.1/CUDA 12.8 pip layer, required the Conda C++ runtime, strengthened model and stage imports, and made the SLURM wrapper select the environment C++ library after activation. | Publish/pull the change, archive the stale Cellpose lock, regenerate it on CSF3, review it, and recreate/test `sbt-cellpose-sam`. | CSF3 diagnosis proved clean SciPy succeeds, Torch 2.13 loads `/usr/lib64/libstdc++.so.6.0.29` and breaks SciPy, while preferring the Conda `libstdc++.so.6.0.34` makes `cellpose.models` pass. All 36 environment-management tests pass locally; repository validation passes with no warnings, including shell syntax and registry/mirror checks. | Pending Linux lock | The committed Cellpose lock predates the declared Pydantic dependency and must not be reused. |
| Modernize `IMC_Denoise_Updated` for a maintainable Python 3.10 runtime | waiting for user | Created branch `codex/python310-tf215`; migrated the package to TensorFlow 2.15.1 through `tf.keras`; added modern package metadata, explicit checkpoint handling, Python 3.10-3.11 constraints, compatibility tests, and README/migration documentation that distinguishes this fork from upstream. | Review the phase, then approve creation and testing of a clean candidate environment. Later, obtain license clarification before any public image containing IMC_Denoise is published. | 11 local tests passed and one TensorFlow-dependent test was skipped because TensorFlow is absent locally; 28 Python files parsed; 4 notebooks passed JSON validation; the lightweight package imported with Python 3.10; a version 1.1.0 wheel built with the expected metadata and package data. | `IMC_Denoise_Updated` branch `codex/python310-tf215`, not committed | TensorFlow execution, model training/loading and representative denoising remain unvalidated until the clean candidate environment exists. The inherited research-only license appears to prohibit redistribution, so public OCI publication is not currently authorized. |
...
- Confirm that cuGraph, dask-cuDF and RAPIDS-singlecell Leiden execute together
  on a CSF3 GPU using the final lock.
...
- Bring the exact final `sbt-analysis` lock (SHA-256
  `9dd5d793b972dce0b69961a466b146ae1fc75ea865954475d6fd0b28305fbec6`)
  from CSF3 into `HPC_env_files/sbt-analysis/conda-linux-64.lock` before
  committing or deploying the standardized catalogue.
