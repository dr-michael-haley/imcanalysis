...
| Correct the RAPIDS/SpatialData Dask-generation conflict | waiting for user | Aligned the candidate to RAPIDS 24.12, RAPIDS-singlecell 0.12.0 and Dask/Distributed 2024.11.2; added cuGraph/dask-cuDF checks, a full GPU clustering smoke artifact, and version-aware Harmony handling that never substitutes Harmony2. Subsequent solver and smoke evidence aligned the candidate to Python 3.11, scikit-image 0.24.0 and Setuptools below 81, and corrected stage smoke tests to use the same process isolation as SLURM. | Review and publish the final corrective specification, regenerate the lock, repair or recreate the candidate, and rerun the strengthened checks on CSF3. | Lock `21141f838d7b93e4ef60f8bd73e8fab726e2994df8ce9299b2aea114fadd64cd` resolved the complete RAPIDS 24.12 stack. Clean installation passed core, PyTorch, RAPIDS/cuGraph/dask-cuDF and all exact-version checks; remaining failures were isolated to Setuptools 84 removing `pkg_resources` and an unrealistic cross-stage same-process import. | Pending | RAPIDS-singlecell 0.12 supports the original Harmony algorithm, not Harmony2. Harmony2 remains available through the legacy RAPIDS environment. |
