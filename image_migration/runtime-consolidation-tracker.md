...
| `sbt-tensorflow` | Candidate shared TensorFlow 2.15 runtime for the modernized IMC-Denoise and HyPERSTAC families; it remains inactive while validation is in progress. |
...
It moves the GPU clustering family to RAPIDS 26.08, RAPIDS-singlecell 0.16.1,
Python 3.12, CUDA 12.9, NumPy 2, Pandas 3 and CuPy 14. Existing RAPIDS stage
mappings remain on `analysis` solely to avoid changing production routing
before the replacement passes direct cuGraph, complete small-workflow and
representative managed-run validation.
...
| Define the dedicated RAPIDS 26.08 candidate | waiting for user | Added an inactive `sbt-rapids` registry entry, a minimal modern dependency specification, CPU-safe API checks, and subprocess-isolated direct cuGraph/full RAPIDS-singlecell GPU smoke tests. | Generate the Linux lock and perform the clean installation on CSF3 in the next approved phase. | All 38 environment-management tests passed; both smoke programs compile; the repository validator passed with zero warnings; `git diff --check` passed. `validate-spec` reports only the intentionally missing Linux lock. No stage mapping or existing lock changed. | Current imcanalysis working tree | Target RAPIDS 26.08, RAPIDS-singlecell 0.16.1, Python 3.12 and CUDA 12.9. Keep `sbt-analysis` unchanged as rollback until acceptance. |
...
| Generate and clean-install `sbt-tensorflow` | waiting for user | Reviewed the resolved lock and isolated the sole post-install failure to login-node GPU discovery in a CPU-only smoke script; now hide GPUs before importing TensorFlow and test that ordering. | Publish and pull the CPU-smoke correction, then rerun `sbt env test tensorflow`; no lock regeneration or environment recreation is required. | Linux lock SHA-256 `9d632d0b42fbbe88fb89fd6b901f5267d8d5c2e91ff08da49b3bcf6593c77860`. Conda, pip extras and the editable overlay installed. Seven registered checks passed, including exact versions, IMC-Denoise, TensorFlow, denoising, HyPERSTAC and the scientific stack. The last CPU calculation failed only because TensorFlow initialized a scheduler-inaccessible GPU on the login node. Locally, all 38 environment-management tests and repository validation pass after the deterministic CPU-only correction. | Current imcanalysis working tree plus CSF3 test log `sbt-tensorflow-tests-20260820-102227.yaml` | GPU calculation and representative scientific workflows remain separate SLURM acceptance steps after the corrected registered suite passes. |
...
candidate intentionally crosses to Python 3.12, NumPy 2, Pandas 3, CuPy 14 and
the current Dask generation. CellCharter's optional TRVAE/scArches route
...
- Run Cellpose-SAM model loading and representative small-image inference on a
  ...
  provenance.
- Publish and pull the deterministic CPU-smoke correction, then rerun the
  registered `sbt-tensorflow` suite. The lock and clean installation already
  succeeded; no recreation is required.
- Validate TensorFlow GPU discovery and a small calculation, then run
...
- Generate and review the `sbt-rapids` Linux lock on CSF3, perform a clean
  installation, and run its registered CPU/import checks.
...
| RAPIDS / CUDA | Transitional RAPIDS 24.12 with CUDA 12.5; replacement candidate RAPIDS 26.08 with CUDA 12.9 | The old generation remains installed only because stage routing has not yet changed. Native Leiden acceptance failed; the dedicated current candidate must pass before activation. |
