...
| Run targeted end-to-end acceptance | in progress | Keep the managed runtime and stage records suitable for representative real workflows. | Continue the current RAPIDS workflow tests and report any stage-specific incompatibility. | Registered and GPU component smoke evidence is complete; representative production workflows remain follow-up evidence rather than an activation gate by explicit decision. | Pending CSF3 run evidence | A failure should be handled as a focused compatibility correction or narrow runtime split, not by silently changing algorithms. |
