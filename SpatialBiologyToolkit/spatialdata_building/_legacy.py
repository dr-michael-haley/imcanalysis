"""Internal SpatialData plotting/write helpers and retired converter code.

The public construction facade is :mod:`SpatialBiologyToolkit.spatialdata`.
The positional converter retained in this private module is not exported and
will be removed after its reusable plotting/write helpers are split out.
"""
...
def get_roi_modalities(sdata: Any, roi: str) -> dict[str, Any]:
    """Return every modality element associated with one ROI.

    The result is driven by schema-3 modality metadata and separates images,
    labels, centroid points, and coordinate systems.  Tables are represented
    in the top-level ``modalities`` mapping because they can span many ROIs.
    """

    selected_roi, selected = _resolve_metadata_roi(sdata, roi)
    metadata = _toolkit_metadata(sdata)
    return {
        "roi": selected_roi,
        "images": dict(selected.get("images", {})),
        "labels": dict(selected.get("label_elements", {})),
        "points": dict(selected.get("points", {})),
        "coordinate_systems": dict(selected.get("coordinate_systems", {})),
        "modalities": dict(selected.get("modalities", {})),
        "primary": dict(metadata.get("primary", {})),
    }


def _casefold_lookup(values: Mapping[str, Any], requested: str, *, kind: str) -> Any:
    matches = [
        value
        for key, value in values.items()
        if str(key).casefold() == str(requested).casefold()
    ]
    if len(matches) != 1:
        available = ", ".join(str(key) for key in values)
        raise KeyError(f"{kind} modality {requested!r} was not found. Available: {available}")
    return matches[0]


def get_roi_elements(
    sdata: Any,
    roi: str,
    *,
    image_modality: str | None = None,
    labels_modality: str | None = None,
) -> dict[str, str | None]:
    """Return selected image, labels, and coordinate-system names for one ROI.

    When a modality is omitted, the primary IMC image and cell-mask modalities
    declared by :class:`SpatialDataSpec` are used.
    """
    ...
    labels = selected.get("label_elements", {})
    selected_image_modality = image_modality or primary.get("imc_images")
    selected_labels_modality = labels_modality or primary.get("cell_masks")
    image = (
        None
        if selected_image_modality is None
        else _casefold_lookup(images, str(selected_image_modality), kind="Image")
    )
    label = (
        selected.get("labels")
        if selected_labels_modality is None
        else _casefold_lookup(labels, str(selected_labels_modality), kind="Labels")
    )
    coordinates = selected.get("coordinate_systems", {})
    coordinate = (
        selected.get("coordinate_system")
        if selected_labels_modality is None
        else _casefold_lookup(
            coordinates,
            str(selected_labels_modality),
            kind="Coordinate-system",
        )
    )
    return {
        "image": None if image is None else str(image),
        "labels": None if label is None else str(label),
        "coordinate_system": None if coordinate is None else str(coordinate),
    }
...
def get_roi_label_elements(sdata: Any, roi: str) -> dict[str, str]:
    ...
    entries use the stable modality names from :class:`RegionLabels` or
    additional :class:`CellMasks` specifications.
    """

    selected_roi, _selected = _resolve_metadata_roi(sdata, roi)
    layers = _toolkit_metadata(sdata).get("label_layers", {})
    result: dict[str, str] = {}
    if isinstance(layers, Mapping):
        for key, raw in layers.items():
            if not isinstance(raw, Mapping):
                continue
            elements = raw.get("elements_by_roi", {})
            if isinstance(elements, Mapping) and selected_roi in elements:
                result[str(key)] = str(elements[selected_roi])
    if not result:
        raise KeyError(f"No label modalities are registered for ROI {selected_roi!r}.")
    return result
...
def summarize_spatialdata(
    ...
    *,
    ...
    table_modality = next(
        (
            raw
            for raw in metadata.get("modalities", {}).values()
            if isinstance(raw, Mapping)
            and str(raw.get("table_name")) == selected_table
        ),
        {},
    )
    roi_key = str(table_modality.get("roi_key", metadata.get("roi_key", "ROI")))
            ...
            sum(
                sum(
                    ...
                    for value in raw.get(
                        "unannotated_instances_by_roi", {}
                    ).values()
                )
                for raw in metadata.get("modalities", {}).values()
                if isinstance(raw, Mapping) and raw.get("kind") == "cell_masks"
            )
        ),
    }
...
def plot_spatialdata_cells(
    ...
    *,
    ...
    image_modality
        Optional IMC image modality name.  The quantified panel linked to the
        selected table is used by default.
    color
    ...
    table_modality: Mapping[str, Any] | None = None
    for raw in metadata.get("modalities", {}).values():
        if isinstance(raw, Mapping) and str(raw.get("table_name")) == selected_table:
            table_modality = raw
            break
    table_masks = (
        None if table_modality is None else table_modality.get("masks")
    )
    selected_image_modality = image_modality or (
        None if table_modality is None else table_modality.get("images")
    )
    if selected_image_modality is None:
        selected_image_modality = metadata.get("primary", {}).get("imc_images")

    roi_region: str | None = None
    if roi is not None:
        _selected_roi, roi_values = _resolve_metadata_roi(sdata, roi)
        label_elements = roi_values.get("label_elements", {})
    ...
    for source_roi, values in metadata.get("rois", {}).items():
        for labels_name in values.get("label_elements", {}).values():
            labels_name = str(labels_name)
    ...
    for gallery_index, position in enumerate(positions):
        ...
        image_name = (
            ...
            if selected_image_modality is None
                ...
                roi_values.get("images", {}),
                str(selected_image_modality),
                kind="Image",
            )
        )
