...
def select_population_cells(
    ...
    *,
    ...
    positive and negative markers. A cell is included only once across strategies.
    ...
    if invalid:
        raise ValueError(f"Unknown strategies {invalid}; choose from {VALID_STRATEGIES}")
    ...
    expected_available = [value for value in expectation_model.markers if value in available]
    ...
    def expression_values(required: Sequence[str] = ()) -> tuple[np.ndarray, dict[str, int]]:
        ...
        if values is None or combined != selected_markers:
            ...
            marker_to_index = {name: index for index, name in enumerate(selected_markers)}
    ...
    if any(value in requested for value in ("core", "boundary")):
        ...
        if structural is None or structural.empty or not required.issubset(structural.columns):
            warnings.append(
            ...
            lookup = pd.Series(
                np.arange(adata.n_obs, dtype=np.int64), index=adata.obs_names.astype(str)
            )
            focused["_position"] = focused["cell_index"].astype(str).map(lookup)
            focused = focused.dropna(subset=["_position"])
            purity_key = next(
                (key for key in ("graph_neighbour_purity", "umap_neighbour_purity") if key in focused),
                None,
            )
    ...
    if selected_marker is None:
        preferred = expectation_model.positive_markers + expectation_model.supportive_markers
        selected_marker = next((value for value in preferred if value in available), None)
    if any(value in requested for value in ("marker_high", "marker_low")):
        if selected_marker is None:
            raise ValueError("marker_high/marker_low requires marker or an expected positive marker")
    ...
    if "contradictory" in requested:
        rule_markers = [value for value in expectation_model.markers if value in available]
        if not rule_markers:
            warnings.append(
            ...
            for name in rule_markers:
                percentile = rankdata(matrix[:, indices[name]], method="average") / len(matrix)
                agreement_terms.append(
                    1 - percentile if name in expectation_model.negative_markers else percentile
                )
    ...
    for strategy in requested:
        ...
        for position, score in zip(positions, scores, strict=False):
            ...
            if instance_key is not None:
                ...
                record[str(instance_key)] = value.item() if hasattr(value, "item") else value
...
__all__ = ["VALID_STRATEGIES", "select_population_cells"]
