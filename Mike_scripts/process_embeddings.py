...
matrix_cmap = {'SMA_fibro_HS': '#1f77b4',
...
for npy, name in zip(['node_embeddings_node2vec.npy', 'node_embedding_graphsage.npy'], ['node2vec','graphsage']):

    # Load embeddings
    node_embeddings = np.load(npy)
    ...
    # Save UMAP
    sc.pl.umap(adatas[name], color=['pixie', 'leiden', 'Case_num', 'area_norm'], s=10, ncols=1, cmap='jet',
               save=f'_{name}.png')

    # Plot and save heatmap
    fig, ax2 = plt.subplots(2, 1, figsize=(5, 5))
    plt.subplots_adjust(hspace=3)
    sb.heatmap(pd.crosstab(adatas[name].obs.leiden, adatas[name].obs.pixie, normalize='index'), ax=ax2.flatten()[0], vmax=0.3)
    ax2.flatten()[0].set_title('Norm across leidens')
    sb.heatmap(pd.crosstab(adatas[name].obs.leiden, adatas[name].obs.pixie, normalize='columns'), ax=ax2.flatten()[1])
